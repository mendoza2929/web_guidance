                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            <div class="row flex-between-center">
                                <div class="col-md">
                                    <h4>Summary</h4>
                                    <textarea name="anecdotal_summary" id="anecdotal_summary" cols="30" rows="10"></textarea>
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                                    <button class="btn btn-primary mt-3" style="text-align: center" id="anecdotalBtn">Save</button>
                                </div>
   ...
   const dropArea = document.getElementById("drop-area");
    ...
    form.addEventListener("submit", (event) => {
        ...
        Array.from(files).forEach((file, index) => {
            ...
            xhr.open("POST", "{{ url('anecdota_upload_image_file') }}", true);
    ...
    $("#anecdotalBtn").click(function(){
    const urlParams = new URLSearchParams(window.location.search);
    const personId = urlParams.get("person_id");

    const formData = new FormData();
    formData.append("person_id", personId);
    formData.append("anecdotal_summary", $("#anecdotal_summary").val());

    $.ajax({
        url: "{{ url('anecdotal_summary') }}",
        type: "POST",
        dataType: 'json',
        contentType: false,
        processData: false,
