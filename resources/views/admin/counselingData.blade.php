            ...
            <div class="card mb-3">
                <div class="card-body">
                    <div class="row flex-between-center">
                        <div class="col-md">
                            <h5 class="mb-2 mb-md-0">Counseling</h5>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            <div class="row flex-between-center">
                                <div class="col-md">
                                    <h4>Summary</h4>
                                    <textarea name="counseling_summary" id="counseling_summary" cols="30" rows="10"></textarea>
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                                    <button class="btn btn-primary mt-3" style="text-align: center" id="counselingBtn">Save</button>
                                </div>
   ...
   const dropArea = document.getElementById("drop-area");
    ...
    form.addEventListener("submit", (event) => {
        ...
        Array.from(files).forEach((file, index) => {
            ...
            xhr.open("POST", "{{ url('counseling_upload_image_file') }}", true);
    ...
    const formData = new FormData();
    formData.append("person_id", personId);
    formData.append("counseling_summary", $("#counseling_summary").val());

    $.ajax({
        url: "{{ url('counseling_summary') }}",
        type: "POST",
        dataType: 'json',
        contentType: false,
        processData: false,
        ...
        error: function(xhr, status, error){
            Swal.fire({
                title: "Error!",
                text: "Unable to save anecdotal summary. Please try again.",
                icon: "error",
                confirmButtonText: "OK"
            });
        }
